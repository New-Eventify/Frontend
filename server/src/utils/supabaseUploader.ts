...
export const uploadToSupabase = async (
  ...
  try {
    ...
    const { error } = await supabase.storage.from(bucket).upload(uniqueFileName, fileBuffer);
    ...
    try {
        ...
        throw new Error(`Failed to retrieve file URL from Supabase: ${error.message}`);
      }
  } catch (error: any) {
